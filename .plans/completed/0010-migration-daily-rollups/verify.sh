...
EXPECTED_0003=(
  daily_engineer_stats
  daily_repo_stats
  daily_review_latency
  daily_review_load
)
EXPECTED_INDEXES=(
  daily_engineer_stats_user_date_idx
  daily_engineer_stats_repo_date_idx
  daily_repo_stats_repo_date_idx
  daily_review_latency_repo_date_idx
  daily_review_load_reviewer_date_idx
)

echo "==> migrate up (fresh — applies 0001, 0002, 0003)"
go run ./cmd/migrate up

echo "==> verify all 0003 tables exist"
for t in "${EXPECTED_0003[@]}"; do
...
for t in "${PRIOR_TABLES[@]}"; do
  if ! sqlite3 "$DB" "SELECT name FROM sqlite_master WHERE type='table' AND name='$t';" | grep -qx "$t"; then
    echo "FAIL: table $t (from prior migration) missing after 0003 up" >&2
    exit 1
  fi
done
...
echo "==> migrate down (one step — drops only 0003)"
go run ./cmd/migrate down

echo "==> verify 0003 tables removed"
for t in "${EXPECTED_0003[@]}"; do
  if sqlite3 "$DB" "SELECT name FROM sqlite_master WHERE type='table' AND name='$t';" | grep -qx "$t"; then
...
echo "  ok: re-up recreated all 0003 tables"
